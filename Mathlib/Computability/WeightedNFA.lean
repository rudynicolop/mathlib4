...
lemma acceptsFrom_inter {M1 : WNFA₂ α σ1 κ} {M2 : WNFA₂ α σ2 κ} {S1 : σ1 →₀ κ} {S2 : σ2 →₀ κ} :
    ...
    = (acceptsFrom M1 S1).hadamard (acceptsFrom M2 S2) := by
  ext x
  simp [WeightedLanguage.hadamard]
  induction x generalizing S1 S2
  case nil =>
...
theorem accepts_inter {M1 : WNFA₂ α σ1 κ} {M2 : WNFA₂ α σ2 κ} :
    accepts (inter M1 M2) = (accepts M1).hadamard (accepts M2) := by
  simp [accepts, ←acceptsFrom_inter]
...
@[simp]
def toWNFA₁start : Multiset (σ × κ) :=
  getMultiset M.start

@[simp]
def toWNFA₁final : σ → κ := M.final.toFun

@[simp]
def toWNFA₁step (s : σ) (a : α) : Multiset (σ × κ) :=
  getMultiset <| M.step s a

@[simps]
def toWNFA₁ : WNFA₁ α σ κ where
  step := toWNFA₁step M
  start := toWNFA₁start M
  final := toWNFA₁final M
